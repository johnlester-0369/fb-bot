    ...
    try {
      ...
      if (reason && typeof reason === "object") {
        ...
        if (errorCode === "UND_ERR_CONNECT_TIMEOUT" ||
          ...
          logger.warn(`Network timeout error caught (non-fatal):`, reason);
        ...
        if (errorCode === "ECONNREFUSED" ||
          ...
          logger.warn(`Network connection error caught (non-fatal):`, reason);
          return; // Don't crash, just log
        }
      }
  ...
  process.on("uncaughtException", (error) => {
    try {
      ...
      if (errorCode === "UND_ERR_CONNECT_TIMEOUT" ||
          ...
          errorMessage.includes("Connect Timeout") ||
          errorMessage.includes("fetch failed")) {
        logger.warn(`Uncaught network timeout error (non-fatal):`,error);
        return; // Don't crash
      }

      // For other uncaught exceptions, log but try to continue
      logger.error(`Uncaught exception (attempting to continue):`,error);
 ...
 * @param {Object} loginData - Login data containing appState, Cookie, email, password
