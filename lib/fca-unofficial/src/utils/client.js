...
function parseAndCheckLogin(ctx, http, retryCount = 0) {
  ...
  const maybeAutoLogin = async (resData, resConfig) => {
    ...
    ctx.auto_login = true;
    logger.warn("Login session expired, attempting auto login...");

    try {
      const ok = await ctx.performAutoLogin();
      if (ok) {
        logger.info("Auto login successful! Retrying request...");
        ...
        if (resConfig) {
          ...
          try {
            ...
            if (retryErr?.code === "ERR_INVALID_CHAR" || (retryErr?.message && retryErr.message.includes("Invalid character in header"))) {
              logger.error(`Auto login retry failed: Invalid header detected. Error:`, retryErr);
            ...
            logger.error(`Auto login retry failed:`, retryErr);
      ...
      // Otherwise, wrap it
      logger.error(`Auto login error:`, autoLoginErr);
  ...
  return async (res) => {
    ...
    if (status >= 500 && status < 600) {
      if (retryCount >= 5) {
        ...
        logger.error(`parseAndCheckLogin: Max retries (5) reached for status ${status}`);
        throw err;
      }
      ...
      logger.warn(`parseAndCheckLogin: Retrying request (attempt ${retryCount + 1}/5) after ${retryTime}ms for status ${status}`);
      ...
      try {
        ...
        // Handle ERR_INVALID_CHAR - don't retry, return error immediately
        if (retryErr?.code === "ERR_INVALID_CHAR" || (retryErr?.message && retryErr.message.includes("Invalid character in header"))) {
          logger.error(`parseAndCheckLogin: Invalid header detected, aborting retry. Error:`, retryErr);
        ...
        if (retryCount >= 5) {
          logger.error(`parseAndCheckLogin: Max retries reached, returning error instead of crashing`);
    ...
    if (resStr.includes("1501092823525282")) {
      logger.error("Bot checkpoint 282 detected, please check the account!");
    ...
    if (resStr.includes("828281030927956")) {
      logger.error("Bot checkpoint 956 detected, please check the account!");
