...
function createSchedulerInstance(api) {
  ...
  function scheduleMessage(message, threadID, when, options = {}) {
    ...
    scheduled.timeout = setTimeout(() => {
      ...
      try {
        logger.info("Sending scheduled message " + id);
        ...
        api.sendMessage(
          ...
          logger.error("Error sending scheduled message " + id + ":", err);
        ...
        logger.error("Error in scheduled message " + id + ":", err);
        scheduledMessages.delete(id);
      }
    }, delay);

    scheduledMessages.set(id, scheduled);
    logger.info("Message scheduled: " + id + " (in " + Math.round(delay / 1000) + "s)");
  ...
  function cancelAllScheduledMessages() {
    ...
    logger.info("Cancelled " + count + " scheduled messages");
    return count;
  }
  ...
  function cleanup() {
    ...
    if (cleaned > 0) {
      logger.info("Cleaned up " + cleaned + " scheduled messages");
    }
  }
  ...
  function destroy() {
    ...
    logger.info("Scheduler destroyed and all resources cleaned up");
    return count;
  }
