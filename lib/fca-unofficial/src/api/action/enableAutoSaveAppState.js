...
module.exports = function (defaultFuncs, api, ctx) {
  return function enableAutoSaveAppState(options = {}) {
    ...
    function saveAppState() {
      try {
        ...
        if (!appState || !appState.appState || appState.appState.length === 0) {
          logger.warn("AppState is empty, skipping save");
          return;
        }

        const data = JSON.stringify(appState, null, 2);
        fs.writeFileSync(filePath, data, "utf8");
        logger.info(`AppState saved to ${filePath}`);
      } catch (error) {
        logger.error(`Error saving AppState:`, error);
      }
    }
    ...
    logger.info(`Auto-save AppState enabled: ${filePath} (every ${Math.round(interval / 1000 / 60)} minutes)`);
    ...
    return function disableAutoSaveAppState() {
      ...
      logger.info("Auto-save AppState disabled");
    };
  };
};