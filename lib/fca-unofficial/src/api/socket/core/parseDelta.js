...
/**
 * Factory function to create parseDelta handler
 * @param {Object} deps - Dependencies
 * @returns {Function} parseDelta function
 */
module.exports = function createParseDelta(deps) {
  const { markDelivery, parseAndCheckLogin } = deps;

  /**
   * Parse delta messages and emit appropriate events
   * @param {Object} defaultFuncs - Default functions
   * @param {Object} api - API object
   * @param {Object} ctx - Context object
   * @param {EventEmitter} emitter - Event emitter
   * @param {Object} param4 - Delta object
   */
  return function parseDelta(defaultFuncs, api, ctx, emitter, { delta }) {
    // Handle new messages
    ...
    // Handle client payload (reactions, unsends, replies)
        ...
        for (const d of clientPayload.deltas) {
          ...
          if (d.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
            const messageReaction = {
              ...
              threadID: (d.deltaMessageReaction.threadKey.threadFbId
                ? d.deltaMessageReaction.threadKey.threadFbId
                : d.deltaMessageReaction.threadKey.otherUserFbId).toString(),
            ...
            emitter.emit("message_reaction", messageReaction);
          }
          // Message unsend
          else if (d.deltaRecallMessageData && !!ctx.globalOptions.listenEvents) {
            const messageUnsend = {
              type: "message_unsend",
              threadID: (d.deltaRecallMessageData.threadKey.threadFbId
                ? d.deltaRecallMessageData.threadKey.threadFbId
                : d.deltaRecallMessageData.threadKey.otherUserFbId).toString(),
            ...
            emitter.emit("message_unsend", messageUnsend);
          }
          // Message reply
            ...
            try {
              ...
              if (!msg || !msg.messageMetadata) {
                logger.warn("parseDelta: deltaMessageReply.message or messageMetadata is missing");
                return;
              }
              ...
              if (d.deltaMessageReply.repliedToMessage) {
                try {
                  ...
                  logger.warn(`parseDelta message_reply repliedToMessage error:`, err);
                }
                ...
                return defaultFuncs.post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, {
                  ...
                  logger.warn(`parseDelta message_reply fetch error:`, err);
                }).finally(() => {
                  if (callbackToReturn) {
                    if (ctx.globalOptions.autoMarkDelivery) {
                    ...
                    emitter.emit("message_reply", callbackToReturn);
                  }
                });
              } else {
              ...
              logger.warn(`parseDelta message_reply error:`, err);
              return;
            }
            if (callbackToReturn) {
              if (ctx.globalOptions.autoMarkDelivery) {
    ...
    switch (delta.class) {
      case "ReadReceipt": {
        ...
        emitter.emit("read_receipt", fmtMsg);
        break;
      }
        ...
        if (mid && tid) {
          ...
          defaultFuncs.post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, form)
            ...
            .then(resData => {
              ...
              logger.warn(`parseDelta ForcedFetch error:`, err);
            });
        }
        break;
      }
      ...
      case "ParticipantLeftGroupThread": {
        ...
        emitter.emit("event", formattedEvent);
        break;
      }
      case "NewMessage": {
