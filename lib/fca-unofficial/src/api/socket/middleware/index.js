...
module.exports = function createMiddlewareSystem() {
  ...
  function use(middleware, fn) {
    ...
    logger.info(`Middleware "${name}" added`);
    ...
    return function remove() {
      ...
      if (index !== -1) {
        ...
        logger.info(`Middleware "${name}" removed`);
      }
    };
  }
  ...
  function remove(identifier) {
    if (typeof identifier === "string") {
      ...
      if (index !== -1) {
        ...
        logger.info(`Middleware "${removed.name}" removed`);
        return true;
      }
      return false;
  ...
  function clear() {
    ...
    logger.info(`All middleware cleared (${count} removed)`);
  }
  ...
  function process(event, finalCallback) {
    ...
    function next(err) {
      ...
      try {
        ...
        logger.error(`Middleware "${middleware.name}" error:`, err);
        next(err);
      }
    }
